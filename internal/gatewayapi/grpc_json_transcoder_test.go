...
func TestBuildGRPCJSONTranscoder(t *testing.T) {
	...
	policy.SetGroupVersionKind(egv1a1.GroupVersion.WithKind("BackendTrafficPolicy"))
	...
	got, err := tr.buildGRPCJSONTranscoder(policy, nil)
	require.NoError(t, err)
	require.Equal(t, "backendtrafficpolicy/default/transcode", got.Name)
...
// A merged policy carries the route policy's ObjectMeta, so a gateway-level transcoder must
// still resolve its ConfigMap in the gateway policy's namespace.
func TestBuildGRPCJSONTranscoderMergedOwner(t *testing.T) {
	newPolicy := func(ns, name string, withTranscoder bool) *egv1a1.BackendTrafficPolicy {
		p := &egv1a1.BackendTrafficPolicy{
			ObjectMeta: metav1.ObjectMeta{Namespace: ns, Name: name},
		}
		if withTranscoder {
			p.Spec.GRPCJSONTranscoder = &egv1a1.GRPCJSONTranscoder{
				ProtoDescriptor: egv1a1.ProtoDescriptor{
					ValueRef: gwapiv1.LocalObjectReference{Kind: "ConfigMap", Name: "descriptor"},
				},
			}
		}
		p.SetGroupVersionKind(egv1a1.GroupVersion.WithKind("BackendTrafficPolicy"))
		return p
	}

	gwPolicy := newPolicy("infra", "gw-policy", true)
	routePolicy := newPolicy("team-a", "route-policy", false)

	tr := &Translator{TranslatorContext: &TranslatorContext{}}
	tr.SetConfigMaps([]*corev1.ConfigMap{
		{
			ObjectMeta: metav1.ObjectMeta{Namespace: "infra", Name: "descriptor"},
			Data:       map[string]string{"proto-descriptor": grpcEchoDescriptorB64},
		},
	})

	// The merged policy is the gateway spec under the route policy's identity.
	merged := newPolicy("team-a", "route-policy", true)

	owners := buildBackendTrafficPolicyOwners(routePolicy, gwPolicy)
	require.Equal(t, gwPolicy, owners.grpcJSONTranscoder,
		"the gateway policy set the field, so it owns it")

	got, err := tr.buildGRPCJSONTranscoder(merged, owners)
	require.NoError(t, err, "must resolve the ConfigMap in the owner's namespace")
	require.Equal(t, "backendtrafficpolicy/infra/gw-policy", got.Name,
		"merged routes must share one filter instance, keyed on the owning policy")

	// When the route policy sets the field, it owns it.
	routeOwns := newPolicy("team-a", "route-policy", true)
	owners = buildBackendTrafficPolicyOwners(routeOwns, gwPolicy)
	require.Equal(t, routeOwns, owners.grpcJSONTranscoder)
}
