...
func testTranscoder(t *testing.T) *ir.GRPCJSONTranscoder {
	return &ir.GRPCJSONTranscoder{
		Name:               "backendtrafficpolicy/default/transcode",
...
func TestPatchHCMGRPCJSONTranscoderSetsDescriptor(t *testing.T) {
	...
	listener := &ir.HTTPListener{
		Routes: []*ir.HTTPRoute{{
			Name:    "route-1",
			Traffic: &ir.TrafficFeatures{GRPCJSONTranscoder: transcoder},
		}},
	}
	...
	require.Equal(t, "envoy.filters.http.grpc_json_transcoder/backendtrafficpolicy/default/transcode", filter.Name)
...
func TestPatchHCMGRPCJSONTranscoderDedupes(t *testing.T) {
	...
	listener := &ir.HTTPListener{
		Routes: []*ir.HTTPRoute{
			{Name: "route-1", Traffic: &ir.TrafficFeatures{GRPCJSONTranscoder: transcoder}},
			{Name: "route-2", Traffic: &ir.TrafficFeatures{GRPCJSONTranscoder: transcoder}},
			{Name: "route-3"},
		},
	}
...
func TestPatchRouteGRPCJSONTranscoder(t *testing.T) {
	transcoder := testTranscoder(t)
	route := &routev3.Route{}
	irRoute := &ir.HTTPRoute{Name: "route-1", Traffic: &ir.TrafficFeatures{GRPCJSONTranscoder: transcoder}}

	require.NoError(t, (&grpcJSONTranscoder{}).patchRoute(route, irRoute, nil))

	cfg := route.GetTypedPerFilterConfig()["envoy.filters.http.grpc_json_transcoder/backendtrafficpolicy/default/transcode"]
